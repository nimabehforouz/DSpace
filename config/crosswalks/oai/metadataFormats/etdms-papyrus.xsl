...
<!-- https://github.com/TexasDigitalLibrary/DSpace/blob/dspace-5_x-TDL-ETD/dspace/config/crosswalks/oai/metadataFormats/etdms1-1.xsl -->
...
<!-- vérifier si c'est une TME UdeM. Pour ce, 2 conditions:
	...
	*ET* au moins un type de document = "Thèse ou mémoire" -->
	...
	<xsl:variable name="estUneTME_UdeM">
		<xsl:for-each
			...
			<xsl:if
				test="contains(normalize-space(.), 'Université de Montréal') and $TME = 'true'">
				<xsl:value-of select="'true'"/>
			</xsl:if>
		</xsl:for-each>
	</xsl:variable>
...
-->
	...
	<xsl:if test="$estUneTME_UdeM = 'true'">
